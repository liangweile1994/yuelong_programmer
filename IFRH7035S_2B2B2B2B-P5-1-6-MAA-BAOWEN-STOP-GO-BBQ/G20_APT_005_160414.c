...
void SM1668_SENT_DATA(u8 send_data)
{
     ...
     for (i =0;i<8;i++)
        {
	D_SCK_Port->ODR &= ~D_SCK_Pin;  //GPIO_WriteLow(GPIOC,SCLK);
        delay(8);
	if ((send_data &  0x01)== 1)  D_SDA_Port->ODR |= D_SDA_Pin; //GPIO_WriteHigh(GPIOC,SDATA); 
	 else      D_SDA_Port->ODR &= ~D_SDA_Pin;//       GPIO_WriteLow(GPIOC,SDATA);
        delay(8);
	D_SCK_Port->ODR |= D_SCK_Pin; //GPIO_WriteHigh(GPIOC,SCLK);
        delay(8);
	send_data >>= 1;
	}
}
void SM1668_SEND_COMMAND(u8 send_command)
{
	...
	D_EN_Port->ODR |= D_EN_Pin; //GPIO_WriteHigh(GPIOE,DISPEN);
        delay(8);
	D_EN_Port->ODR &= ~D_EN_Pin; //GPIO_WriteLow(GPIOE,DISPEN);
        delay(8);
	 for(i =0;i<8;i++)
	  {
	  D_SCK_Port->ODR &= ~D_SCK_Pin; //GPIO_WriteLow(GPIOC,SCLK);
        delay(8);
	  if((send_command & 0x01)== 1) D_SDA_Port->ODR |= D_SDA_Pin; //GPIO_WriteHigh(GPIOC,SDATA); 
	   else             D_SDA_Port->ODR &= ~D_SDA_Pin; //       GPIO_WriteLow(GPIOC,SDATA);
        delay(8);
	  D_SCK_Port->ODR |= D_SCK_Pin; //GPIO_WriteHigh(GPIOC,SCLK);
        delay(8);
