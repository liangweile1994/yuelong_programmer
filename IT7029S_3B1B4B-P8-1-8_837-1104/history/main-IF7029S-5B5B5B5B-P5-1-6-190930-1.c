...
const u8 arr_T_Sens_H[16]={  8,     5,    6,    6,  6,   7,   7,   10,   10,   8,  10, 10,   10,   10,    10, 10};
//------------=           VIF2,boost2,Tdec2,Tinc2,ON2,DEC2,INC2,DOUX2,DOUX1,INC1,DEC1,ON1,TINC1,TDEC1,boost1,VF1
...
const u8 arr_T_Sens_L[16]={255,255, 10, 10,255,255, 10,    10,10,    10, 255, 255,255,  10,  10, 255};
//------------=                    ON3,ON4,         F2,TIMER2,F1,TIMER1,               DEC3,DEC4
//APT- T1_SDA & T1_SCK---------
...
const u8 arr_T_Sens_AH[16]={    10,  10,  10,   10,   10,   10,    10,  10,    10,    10,  10,   10,   10,   10,   10,  10};
//------------=             boost4,VIF4,DOUX4,INC4,TDEC4,ONOFF,TIMER4,TINC4,TINC3,TIMER3,LOCK,TDEC3,boost3,VIF3,DOUX3,INC3
/*private prototypes ******************************************************/
//  
...
u16 ON_disp_zone_x(u8 onoff,u8 warm_disp,u8 index,u8 zone_x_flash,u8 BBQ,u8 level,u8 *change,u8 index_f,u8 select,u8 flash,u8 pan,u8 other_pan,u8 err)
{
   ...
   if((*change) &select)
   {
   	if(zone_x_flash %2) dis_buf =CHR_P; 
   ...
   if((zone_x_flash ==0)||((zone_x_flash %2)==0)) 
   {
	if((level)||(warm_disp))   
             {
             	dis_buf = Display_decode(level/10)<<8;
		...
		if(zone_x_flash ==0)
        ...
        else 
             {
             	if((zone_x_flash ==0)&&(flash)&&(Time_dot_bz ==TRUE)) dis_buf = CHR_H;
                	...
                	{
                	if(zone_x_flash)  dis_buf = (NUM_0<<8)|NUM_0;            	
                	else if(onoff)    dis_buf = (SEG_G<<8)|SEG_G;
                	}               
             }
        if(err)
            {
...
{ 
  u8  key_effective =0;
              ...
              if(key_effective)
                {
		   		...
		   		if((key_scan_value_H == VALUE_VIF2)&&(key_scan_value_L == 0x0000)&&(key_scan_value_AH ==0x0000))    	key_ok_bit =KEY_VIF2; 
		   	...
		   	else	if((key_scan_value_H == VALUE_TINC2)&&(key_scan_value_L == 0x0000)&&(key_scan_value_AH ==0x0000))	key_ok_bit =KEY_TINC2;  
                   	...
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L == VALUE_F2)&&(key_scan_value_AH ==0x0000))	    	key_ok_bit =KEY_F2; 
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L == VALUE_TIMER2)&&(key_scan_value_AH ==0x0000))	key_ok_bit =KEY_TIMER2;   
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L == VALUE_F1)&&(key_scan_value_AH ==0x0000))	    	key_ok_bit =KEY_F1;  
                   	...
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L ==0x0000)&&(key_scan_value_AH == VALUE_BOOST4))	key_ok_bit =KEY_BOOST4;  
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L ==0x0000)&&(key_scan_value_AH == VALUE_VIF4))	    	key_ok_bit =KEY_VIF4;
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L ==0x0000)&&(key_scan_value_AH == VALUE_DOUX4))	key_ok_bit =KEY_DOUX4;
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L ==0x0000)&&(key_scan_value_AH == VALUE_INC4))	    	key_ok_bit =KEY_INC4; 
                   	...
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L ==0x0000)&&(key_scan_value_AH == VALUE_BOOST3))	key_ok_bit =KEY_BOOST3;
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L ==0x0000)&&(key_scan_value_AH == VALUE_VIF3))	    	key_ok_bit =KEY_VIF3; 
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L ==0x0000)&&(key_scan_value_AH == VALUE_DOUX3))	key_ok_bit =KEY_DOUX3;
                   	else	if((key_scan_value_H ==0x0000)&&(key_scan_value_L ==0x0000)&&(key_scan_value_AH == VALUE_INC3))	    	key_ok_bit =KEY_INC3;                                  
                   	else  
                   	{
...
void  inc_x_key_level(u8 *count,u8 *zone_flash,u8 *level,u8 *o_level,u16 time1,u16 *time2,u8 select,u16 *tim1,u16 *tim2,u8 *tiao)
{	
  	if(time1 ==0)
  	{
  	  if((*count) ==0)		(*count) =1;  
...
void  dec_x_key_level(u8 *count,u8 *zone_flash,u8 *level,u8 *o_level,u16 time1,u16 *time2,u8 select, u16 *tim1,u16 *tim2,u8 *tiao)
{
   	if(time1 ==0)
  	{
  	  if((*count) ==0)		(*count) =1;  
...
void on_x_set(u8 *status,u8 *level)
{
...
void boost_x_set(u8 status,u8 select,u8 *level1,u8 *level2,u16 *time,u16 *o_time,u16 *tim1)
{   
...
void vif_x_set(u8 status,u8 select,u8 *level1,u16 *o_time,u16 *tim1)
{
    if(status)
    {
         buzzer_time = buz_200ms;
         {
           if((*o_time)==0) (*level1) =14; 
           else {change_count =select;(*o_time) =0;(*level1) =8;} 
         }
       if((timer_status & select)==0) (*tim1)  =arr_default_timer[(*level1)];		    		           
    }
}
void doux_x_set(u8 status,u8 select,u8 *level1,u16 *o_time,u16 *tim1)
{
    if(status)
    {
         buzzer_time = buz_200ms;
        ...
        if(Ms10_bz4 == TRUE)
         {
        	...
        	key_next_deal();
          	...
          	if((Power_on_bz == TRUE) && (lock_bz == FALSE)&&(key_account == short_press_time) &&(Pause_bz ==0)) 
          	 {
          	 	//------------------------------------------------  
          	 	if(key_ok_bit == KEY_ON1) {on_x_set(&on1_bz,&zone_1_level);speed_up();} 
          	 	if(key_ok_bit == KEY_ON2) {on_x_set(&on2_bz,&zone_2_level);speed_up();}  
          	 	if(key_ok_bit == KEY_ON3) {on_x_set(&on3_bz,&zone_3_level);speed_up();} 
          	 	if(key_ok_bit == KEY_ON4) {on_x_set(&on4_bz,&zone_4_level);speed_up();}           	 	
          	 	//------------------------------------------------  
          	 	if((on1_bz)&&(key_ok_bit == KEY_DEC1))  {dec_x_key_level(&count_1_key,&zone_flash_count1,&zone_1_level,&zone_2_level,boost_time_1,&boost_time_2,Select_1_KEY,&zone_1_timer,&zone_2_timer,&tiao_on_1);speed_up();}
          	 	if((on2_bz)&&(key_ok_bit == KEY_DEC2))  {dec_x_key_level(&count_2_key,&zone_flash_count2,&zone_2_level,&zone_1_level,boost_time_2,&boost_time_1,Select_2_KEY,&zone_2_timer,&zone_1_timer,&tiao_on_2);speed_up();}
          	 	if((on3_bz)&&(key_ok_bit == KEY_DEC3))  {dec_x_key_level(&count_3_key,&zone_flash_count3,&zone_3_level,&zone_4_level,boost_time_3,&boost_time_4,Select_3_KEY,&zone_3_timer,&zone_4_timer,&tiao_on_3);speed_up();} 
          	 	if((on4_bz)&&(key_ok_bit == KEY_DEC4))  {dec_x_key_level(&count_4_key,&zone_flash_count4,&zone_4_level,&zone_3_level,boost_time_4,&boost_time_3,Select_4_KEY,&zone_4_timer,&zone_3_timer,&tiao_on_4);speed_up();}
          		//------------------------------------------------  
          	 	if((on1_bz)&&(key_ok_bit == KEY_INC1))  {inc_x_key_level(&count_1_key,&zone_flash_count1,&zone_1_level,&zone_2_level,boost_time_1,&boost_time_2,Select_1_KEY,&zone_1_timer,&zone_2_timer,&tiao_on_1);speed_up();} 
          	 	if((on2_bz)&&(key_ok_bit == KEY_INC2))  {inc_x_key_level(&count_2_key,&zone_flash_count2,&zone_2_level,&zone_1_level,boost_time_2,&boost_time_1,Select_2_KEY,&zone_2_timer,&zone_1_timer,&tiao_on_2);speed_up();} 
          	 	if((on3_bz)&&(key_ok_bit == KEY_INC3))  {inc_x_key_level(&count_3_key,&zone_flash_count3,&zone_3_level,&zone_4_level,boost_time_3,&boost_time_4,Select_3_KEY,&zone_3_timer,&zone_4_timer,&tiao_on_3);speed_up();}
          	 	if((on4_bz)&&(key_ok_bit == KEY_INC4))  {inc_x_key_level(&count_4_key,&zone_flash_count4,&zone_4_level,&zone_3_level,boost_time_4,&boost_time_3,Select_4_KEY,&zone_4_timer,&zone_3_timer,&tiao_on_4);speed_up();}
          		//------------------------------------------------ 
          		if(key_ok_bit == KEY_DOUX1)  {doux_x_set(on1_bz,Select_1_KEY,&zone_1_level,&boost_time_2,&zone_1_timer);speed_up();} 
          		if(key_ok_bit == KEY_DOUX2)  {doux_x_set(on2_bz,Select_2_KEY,&zone_2_level,&boost_time_1,&zone_2_timer);speed_up();} 
          		if(key_ok_bit == KEY_DOUX3)  {doux_x_set(on3_bz,Select_3_KEY,&zone_3_level,&boost_time_4,&zone_3_timer);speed_up();} 
          		if(key_ok_bit == KEY_DOUX4)  {doux_x_set(on4_bz,Select_4_KEY,&zone_4_level,&boost_time_3,&zone_4_timer);speed_up();} 
          		//------------------------------------------------ 
          		if(key_ok_bit == KEY_VIF1)   {vif_x_set(on1_bz,Select_1_KEY,&zone_1_level,&boost_time_2,&zone_1_timer);speed_up();} 
          		if(key_ok_bit == KEY_VIF2)   {vif_x_set(on2_bz,Select_2_KEY,&zone_2_level,&boost_time_1,&zone_2_timer);speed_up();} 
          		if(key_ok_bit == KEY_VIF3)   {vif_x_set(on3_bz,Select_3_KEY,&zone_3_level,&boost_time_4,&zone_3_timer);speed_up();} 
          		if(key_ok_bit == KEY_VIF4)   {vif_x_set(on4_bz,Select_4_KEY,&zone_4_level,&boost_time_3,&zone_4_timer);speed_up();} 
          		//------------------------------------------------ 
          		if(key_ok_bit == KEY_BOOST1) {boost_x_set(on1_bz,Select_1_KEY,&zone_1_level,&zone_2_level,&boost_time_1,&boost_time_2,&zone_1_timer);speed_up();} 
          		if(key_ok_bit == KEY_BOOST2) {boost_x_set(on2_bz,Select_2_KEY,&zone_2_level,&zone_1_level,&boost_time_2,&boost_time_1,&zone_2_timer);speed_up();} 
          		if(key_ok_bit == KEY_BOOST3) {boost_x_set(on3_bz,Select_3_KEY,&zone_3_level,&zone_4_level,&boost_time_3,&boost_time_4,&zone_3_timer);speed_up();} 
          		if(key_ok_bit == KEY_BOOST4) {boost_x_set(on4_bz,Select_4_KEY,&zone_4_level,&zone_3_level,&boost_time_4,&boost_time_3,&zone_4_timer);speed_up();} 
          		//------------------------------------------------        	 	 
            ...
            if(Power_on_bz ==FALSE)  				  //off status
               	 	 ...
               	 	 {               	   
               	 			for(i=0;i<16;i++)   Disp_buf[i] =CHR_BLANK;	     	//all blank
               	   			...
               	   			if((idle_count) && (lock_bz == TRUE))  	  //"Lo"			    
               	    			...
               	    			if(Time_dot_bz ==TRUE)
               	    			{
               	    			if(flash_H_1_min) Disp_buf[7] = CHR_H;  //flash "H" 
               	    			if(flash_H_2_min) Disp_buf[8] = CHR_H;  //flash "H"  
               	    			if(flash_H_3_min) Disp_buf[10] =CHR_H;	//flash "H"  
               	    			if(flash_H_4_min) Disp_buf[5] = CHR_H;  //flash "H"  
               	    			}  			           	 		                       	    		    			             	         
               	  	}                                                        
        	}                                                                
            else	//on status                                              
            	{             		
     	 		...
     	 		Disp_buf[0] =temp_16>>8;
               		...
               		Disp_buf[3] =temp_16>>8;
               		Disp_buf[7] =temp_16&0x00FF;
               		...
               		Disp_buf[15] =temp_16>>8;
              	 	Disp_buf[8] =temp_16&0x00FF;
               		...
               		temp_16=ON_disp_zone_x(on3_bz,(Warm_status &Select_3_KEY),2,zone_flash_count3,bbq_R_bz,zone_3_level,&change_count,flex_R_bz,Select_3_KEY,flash_H_3_min,zone_3_nopan_time,zone_4_nopan_time,Err_3_data);	 //zone3 normal display               		
               	  	Disp_buf[13] =temp_16>>8;
               	  	Disp_buf[10] =temp_16&0x00FF;
               		...
               		temp_16=ON_disp_zone_x(on4_bz,(Warm_status &Select_4_KEY),3,zone_flash_count4,bbq_R_bz,zone_4_level,&change_count,flex_R_bz,Select_4_KEY,flash_H_4_min,zone_4_nopan_time,zone_3_nopan_time,Err_4_data);	 //zone4 normal display 			
               	  	Disp_buf[1] =temp_16>>8;
               	  	Disp_buf[5] =temp_16&0x00FF;
 			//
 			...
 			//               	        
               	        if(Pause_bz) 
               	           {
               	             Disp_buf[2] =SEG_B|SEG_C|SEG_E|SEG_F;
               	             Disp_buf[6] =SEG_B|SEG_C|SEG_E|SEG_F;
               	             Disp_buf[9] =SEG_B|SEG_C|SEG_E|SEG_F;
               	             Disp_buf[14] =SEG_B|SEG_C|SEG_E|SEG_F;
               	             Disp_buf[11] =SEG_B|SEG_C|SEG_E|SEG_F;
               	             Disp_buf[12] =SEG_B|SEG_C|SEG_E|SEG_F;
               	             Disp_buf[4] =SEG_B|SEG_C|SEG_E|SEG_F;
               	             Disp_buf[0] =SEG_B|SEG_C|SEG_E|SEG_F;
               	           }
                }  
                /*
                disp_debug++;                
                ...
                Disp_buf[8] =Display_decode((disp_debug & 0x000000000F000000)>>24);
                Disp_buf[14] =Display_decode((disp_debug & 0x0000000000F00000)>>20);
                Disp_buf[9] =Display_decode((disp_debug & 0x00000000000F0000)>>16);
                Disp_buf[13] =Display_decode((disp_debug & 0x000000000000F000)>>12);
                Disp_buf[10]=Display_decode((disp_debug & 0x0000000000000F00)>>8);
                Disp_buf[12] =Display_decode((disp_debug & 0x00000000000000F0)>>4);
                Disp_buf[11] =Display_decode( disp_debug & 0x000000000000000F);
                ...
                Disp_buf[1] =Display_decode((disp_debug & 0x000000000000F000)>>12);
                Disp_buf[5]=Display_decode((disp_debug & 0x0000000000000F00)>>8);
                Disp_buf[0] =Display_decode((disp_debug & 0x00000000000000F0)>>4);
...
//************************************************                  	
	if(Flash_500ms_bz == TRUE)
	   ...
	   if(zone_flash_count4 ==0) count_4_key =0;
           ...
           if((zone_flash_count1 ==0) && (time_flash_count1 ==0))   key_zone1 =0; 
           if((zone_flash_count2 ==0) && (time_flash_count2 ==0))   key_zone2 =0; 
           if((zone_flash_count3 ==0) && (time_flash_count3 ==0))   key_zone3 =0; 
           if((zone_flash_count4 ==0) && (time_flash_count4 ==0))   key_zone4 =0; 	 
