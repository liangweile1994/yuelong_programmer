...
u8 ON_disp_zone_x(u8 warm_disp,u8 index,u8 zone_x_flash,u8 BBQ,u8 level,u8 *change,u8 index_f,u8 select,u8 flash,u8 pan,u8 other_pan,u8 err)
{
   ...
   //if((key_zone != select) || ((zone_x_flash %2)==0)) 
   {
	if((level)||(warm_disp))   
             {
	...
	if(Prepare_data_bz ==TRUE)
	{
	   ...
	   if(Time_dot_bz ==TRUE) Time_dot_bz =FALSE;
            ...
            if(Power_on_bz ==FALSE)  				  //off status
               	 	 ...
               	 	 {               	   
               	 			for(i=0;i<16;i++)   Disp_buf[i] =CHR_BLANK;	     	//all blank
               	   			...
               	   			if((idle_count) && (lock_bz == TRUE))  	  //"Lo"			    
               	    			...
               	    			if(Time_dot_bz ==TRUE)
               	    			{
               	    			if(flash_H_1_min) Disp_buf[7] = CHR_H;  //flash "H"  COM5
               	    			if(flash_H_2_min) Disp_buf[8] = CHR_H;  //flash "H"  COM4
               	    			if(flash_H_3_min) Disp_buf[10] = CHR_H;	//flash "H"  COM2
               	    			if(flash_H_4_min) Disp_buf[5] = CHR_H;  //flash "H"  COM3
               	    			}  			           	 		                       	    		    			             	         
               	  	}                                                        
        	}                                                                
            else	//on status                                              
            	{             		
               	   	{
               	 		if(time_flash_count1)				//flash on
               	 			{
     	 			    	 //disp_timer_1();
               	 			}
               	 		if(time_flash_count1) {Disp_buf[0] = NUM_0;Disp_buf[1] = NUM_0; }   
               	    				{
               	    				...
               	    				Disp_buf[1] =CHR_BLANK;
               	    		 		...
               	    		 		Disp_buf[0] = SEG_G;
               	 		 	 	Disp_buf[1] = SEG_G;
               	    		 		}   									//not flash ,display normal,calculate the min timer zone 
               	   	...
               	   	if(zone_1_level ==0) {Disp_buf[3] =CHR_BLANK;Disp_buf[7] =CHR_BLANK;}
               	   	else
               	   	{ 
               	   	Disp_buf[3] =Display_decode(zone_1_level/10);	            	   	
               	  	Disp_buf[7] =ON_disp_zone_x((Warm_status &Select_1_KEY),0,zone_flash_count1,bbq_L_bz,zone_1_level%10,&change_count,flex_L_bz,Select_1_KEY,flash_H_1_min,zone_1_nopan_time,zone_2_nopan_time,Err_1_data);	 //zone1 normal display
               		}
               		if(zone_2_level ==0) {Disp_buf[15] =CHR_BLANK;Disp_buf[8] =CHR_BLANK;} 
               	        else
               	        {
               	        Disp_buf[15] =Display_decode(zone_2_level/10);
              	 	Disp_buf[8] =ON_disp_zone_x((Warm_status &Select_2_KEY),1,zone_flash_count2,bbq_L_bz,zone_2_level,&change_count,flex_L_bz,Select_2_KEY,flash_H_2_min,zone_2_nopan_time,zone_1_nopan_time,Err_2_data);	 //zone2 normal display               	
               		}
               		if(zone_3_level ==0) {Disp_buf[13] =CHR_BLANK;Disp_buf[10] =CHR_BLANK;} 
               	  	else
               	  	{
               	  	Disp_buf[13] =Display_decode(zone_3_level/10);
               	  	Disp_buf[10] =ON_disp_zone_x((Warm_status &Select_3_KEY),2,zone_flash_count3,bbq_R_bz,zone_3_level,&change_count,flex_R_bz,Select_3_KEY,flash_H_3_min,zone_3_nopan_time,zone_4_nopan_time,Err_3_data);	 //zone3 normal display
               		}
               		if(zone_4_level ==0) {Disp_buf[1] =CHR_BLANK;Disp_buf[5] =CHR_BLANK;} 
               	  	else
               	  	{
               	  	Disp_buf[1] =Display_decode(zone_4_level/10);
               	  	Disp_buf[5] =ON_disp_zone_x((Warm_status &Select_4_KEY),3,zone_flash_count4,bbq_R_bz,zone_4_level,&change_count,flex_R_bz,Select_4_KEY,flash_H_4_min,zone_4_nopan_time,zone_3_nopan_time,Err_4_data);	 //zone4 normal display
 			}
 			//
