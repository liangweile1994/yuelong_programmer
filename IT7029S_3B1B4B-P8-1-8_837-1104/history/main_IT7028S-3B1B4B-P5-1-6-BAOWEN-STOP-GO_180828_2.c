...
const u16 arr_default_timer[10]  = {0,480,480,480,240,240,240,120,120,120};        //minitue
		//0.73*(arr_value -1)=S(ton)    
		//0.73*(11+1) =8.76S =S(tzong)
	     	//    1,    2,    3,    4,    5,    6
	     	//2.19s,2.92s,3.65s,4.38s,5.11s,6.57s
const u8 arr_tiao_on[6]={4,    5,    6,    7,    8,   10};    //erp
const u8 arr_tiao_180_pwm[6] ={CON_PWM_8us,CON_PWM_8us,CON_PWM_9us,CON_PWM_10us,CON_PWM_10us,CON_PWM_10us};    
		  // 1#,     2#,       3#,     4#
const u8 erp_pwm[4] ={pwm_210,pwm_160,pwm_280,pwm_160};//erp
		    //   7   8   9   P
		    // 1k3,1k8,2k0,2k6
const u8 arr_1_pow[4]={ 74,102,113,158};	//180mm 58.9uH	  *1.055 for D358  //70, 76, 86, 97,108,143
		    // 1k3,1k8,2k5,3k6
const u8 arr_3_pow[4]={ 74,102,145,210};	//280mm 58.9uH	  *1.055 for D358  //70, 76, 86, 97,108,143
		    // 1k2,1k2,1k5,1k9
const u8 arr_2_pow[4]={123,133,170,230};  	//180mm 68uH      *1.065 for D358  //120,130,145,160,175,230
const u8 arr_4_pow[4]={123,133,170,230};  	//180mm 68uH      *1.065 for D358  //120,130,145,160,175,230  
			//      5          6    	   7     	8     	  9    	       P
...
const u8 arr_T_Sens_H[16]={255,255, 6, 7, 6, 5, 3, 3, 3, 3,  3,  10,10, 11, 18,255};
...
const u8 arr_T_Sens_L[16]={255, 11, 13, 10,  8,  3,  3,255,  3,  4,  4,  6,  8,  6,255,255};
...
void comm_get_deal(u8 index)
{
	...
	u8 *err =&Err_1_data +index; 
       ...
       //2# 4# IH heating NTC 
     if(index >=0)	//  //2# 3# 4#special deal temperature  ((index ==1)||(index==3))
      {
...
u8 ON_disp_zone_x(u8 warm_disp,u8 index,u8 BBQ,u8 level,u8 *change,u8 index_f,u8 select,u8 flash,u8 pan,u8 other_pan,u8 err)
{
	...
	if((level)||(warm_disp))   
             {
               	...
               	if(BBQ)  
               	{
               	if((index ==1)||(index ==2))   dis_buf =CHR_B;	
               	if((index ==0)||(index ==3))   dis_buf =CHR_Q;
        	}
               	// 
...
{ 
  u8  key_effective =0;
              ...
              if(key_effective)
		   	...
		   	if(key_scan_value == VALUE_T2)	    key_ok_bit =KEY_T2; 
		   else	if(key_scan_value == VALUE_T3)	    key_ok_bit =KEY_T3; 
		   else	if(key_scan_value == VALUE_T4)	    key_ok_bit =KEY_T4;   
		   else	if(key_scan_value == VALUE_T5)	    key_ok_bit =KEY_T5;    
		   else	if(key_scan_value == VALUE_T6)	    key_ok_bit =KEY_T6; 
		   else	if(key_scan_value == (VALUE_T5|VALUE_T6)) key_ok_bit =KEY_T6;
                   else	if(key_scan_value == VALUE_T7)	    key_ok_bit =KEY_T7; 
                   else	if(key_scan_value == (VALUE_T6|VALUE_T7)) key_ok_bit =KEY_T7;
                   else	if(key_scan_value == VALUE_T8)	    key_ok_bit =KEY_T8; 
                   else	if(key_scan_value == (VALUE_T7|VALUE_T8)) key_ok_bit =KEY_T8;
                   else	if(key_scan_value == VALUE_T9)	    key_ok_bit =KEY_T9; 
                   else	if(key_scan_value == (VALUE_T8|VALUE_T9)) key_ok_bit =KEY_T9;
		   else	if(key_scan_value == VALUE_T10)           key_ok_bit =KEY_T10; 
		   else	if(key_scan_value == (VALUE_T9|VALUE_T10)) key_ok_bit =KEY_T10;
		   else	if(key_scan_value == VALUE_T11)	    key_ok_bit =KEY_T11; 
                   else	if(key_scan_value == VALUE_T12)	    key_ok_bit =KEY_T12; 
                   else	if(key_scan_value == VALUE_T13)	    key_ok_bit =KEY_T13; 
		   else	if(key_scan_value == VALUE_T14)           key_ok_bit =KEY_T14; 

                   else if(key_scan_value == VALUE_T15)	    key_ok_bit =KEY_T15; 
                   else	if(key_scan_value == VALUE_T16)	    key_ok_bit =KEY_T16; 
                   else	if(key_scan_value == VALUE_T17)	    key_ok_bit =KEY_T17; 
                   else	if(key_scan_value == VALUE_T18)	    key_ok_bit =KEY_T18;
                   else	if(key_scan_value == VALUE_T19)	    key_ok_bit =KEY_T19; 
                   else	if(key_scan_value == VALUE_T20)	    key_ok_bit =KEY_T20; 
                   else	if(key_scan_value == (VALUE_T19|VALUE_T20))	    key_ok_bit =KEY_T20; 
                   else	if(key_scan_value == VALUE_T21)	    key_ok_bit =KEY_T21;           
                   else	if(key_scan_value == VALUE_T22)	    key_ok_bit =KEY_T22;  
                   else	if(key_scan_value == (VALUE_T20|VALUE_T22))	    key_ok_bit =KEY_T22;                   
                   else	if(key_scan_value == VALUE_T23)	    key_ok_bit =KEY_T23; 
                   else	if(key_scan_value == (VALUE_T22|VALUE_T23))	    key_ok_bit =KEY_T23;                                         
                   else	if(key_scan_value == VALUE_T24)	    key_ok_bit =KEY_T24;
                   else	if(key_scan_value == (VALUE_T23|VALUE_T24))	    key_ok_bit =KEY_T24;  
                   else if(key_scan_value == VALUE_T25)           key_ok_bit =KEY_T25;
                   else	if(key_scan_value == VALUE_T26)	    key_ok_bit =KEY_T26; 
                   else if(key_scan_value == VALUE_T27)           key_ok_bit =KEY_T27; 
                   else if(key_scan_value == (VALUE_T10|VALUE_T19)) key_ok_bit =KEY_T19;                                       
                   else
                   {
                    if(key_scan_value &VALUE_T14)  			key_ok_bit =KEY_T14;                    
                    else
                   	{
                   	key_err_account++;
...
u8  Zone_Big_Set(u8 index,u8 level,u8 count,u8 data,u16 tw)  
{
      if(level ==10)
	{
	   if(index ==3)
	   {					// 2600W
	   if(count ==1)  data -=20;		//dec 400w 
	   if(count ==2)  data -=35;		//dec 600w 
	   if(count >2)   data -=45;		//dec 600w 
	   }	
	   //else
	   {
         ...
         if(index <=7)   
         	{
         		switch(index)
         		{
         			case 0:disp10 =SEG_A; break;
         			case 1:disp10 =SEG_A|SEG_B; break;
         			case 2:disp10 =SEG_A|SEG_B|SEG_C; break;
         			case 3:disp10 =SEG_A|SEG_B|SEG_C|SEG_D; break;
         			case 4:disp10 =SEG_A|SEG_B|SEG_C|SEG_D|SEG_E; break;
         			case 5:disp10 =SEG_A|SEG_B|SEG_C|SEG_D|SEG_E|SEG_F; break;
         			case 6:disp10 =SEG_A|SEG_B|SEG_C|SEG_D|SEG_E|SEG_F|SEG_G; break;
         			case 7:disp10 =SEG_A|SEG_B|SEG_C|SEG_D|SEG_E|SEG_F|SEG_G|SEG_P; break;
         		}
         		disp12 &=~(SEG_A|SEG_B|SEG_C);
         		disp_time =11;	
         	}
         else if(index ==8)   {disp10 =0xFF;disp12 |=SEG_A;disp_time =11;}
         else if(index ==9)   {disp10 =0xFF;disp12 |=SEG_A|SEG_B|SEG_C;disp_time =11;}
      	 //
}
        ...
        if(Ms10_bz4 == TRUE)
         {
        	...
        	key_next_deal();
          	//------------------------------------------------ 
          	if(key_ok_bit == KEY_T14)  {on_off_key_deal();speed_up();} 
          	//------------------------------------------------ 
          	if(key_ok_bit == KEY_T13)  {lock_key_deal();speed_up();}
          	//------------------------------------------------   
          	if(key_ok_bit == KEY_T12)  {Pause_function_key();speed_up();}
          	//------------------------------------------------          		
          	if((Power_on_bz == TRUE) && (lock_bz == FALSE)&&(key_account == short_press_time) &&(Pause_bz ==0)) 
          	 {          	 	
          	 	//------------------------------------------------  
          	 	if(key_ok_bit == KEY_T18)          	
          		{          		
          	       	boost_key_deal();
          	       	speed_up();		
          		}
          		//------------------------------------------------  
          	 	if(key_ok_bit == KEY_T21)          	
          		{          		
          	       	timer88_key_deal();
          	       	speed_up();		
          		} 
          		//------------------------------------------------  
          	 	if(key_ok_bit == KEY_T16)          	
          		{          		
          	       	timer_key_deal();
          	       	speed_up();		
          		} 
          		//------------------------------------------------ 
          		if(key_ok_bit == KEY_T11)	
          		{         		 
          	        Warm_func_x_set(key_zone);
          	        speed_up();	
          		} 
          		//------------------------------------------------ 
          		/*if(key_ok_bit == KEY_T17)	
          		{         		 
          	        BBQ_key_deal();	
          	        speed_up();	
          		}*/ 
          		//------------------------------------------------ 
          		/*if(key_ok_bit == KEY_T15)	
          		{         		 
          	        auto_key_deal();	
          	        speed_up();	
          		}*/          		
          	 	//------------------------------------------------       		 
        		/*if(key_ok_bit == KEY_T3)
          		{
          	       	flex_L_key_deal();  
          	       	speed_up();		
          		}
          		//------------------------------------------------       		 
        		if(key_ok_bit == KEY_T26)
          		{
          	       	flex_R_key_deal();  
          	       	speed_up();		
          		}*/
          		//------------------------------------------------        	 	 
          		 if(key_ok_bit == KEY_T4)
          	 	{
          	 	key_x_deal(Select_1_KEY); 
          	 	speed_up();  
          		}    	 	
          		//------------------------------------------------  
          		if(key_ok_bit == KEY_T2)
          	 	{  
          	 	key_x_deal(Select_2_KEY); 
          	 	speed_up();
          	 	}
          		//------------------------------------------------  
          		if(key_ok_bit == KEY_T27)
          	 	{  
          	 	key_x_deal(Select_3_KEY); 
          	 	speed_up();
          		}
          		//------------------------------------------------  
          		/*if(key_ok_bit == KEY_T25)
          	 	{  
          	 	key_x_deal(Select_4_KEY); 
          	 	speed_up();
          	 	}*/
          	 	//------------------------------------------------  
          		/*if(key_ok_bit == KEY_Txx)
          	 	{  
          	 	Sdec_key_deal(); 
          	 	speed_up();
          	 	}*/       
          	 	//------------------------------------------------  
          		if(key_ok_bit == KEY_T5)
          	 	{  
          	 	slide_x_deal(0);
          	 	speed_up();
          	 	}  
          	 	//------------------------------------------------  
          		if(key_ok_bit == KEY_T6)
          	 	{  
          	 	slide_x_deal(1);
          	 	speed_up();
          	 	}  
          	 	//------------------------------------------------  
          		if(key_ok_bit == KEY_T7)
          	 	{  
          	 	slide_x_deal(2);
          	 	speed_up();
          	 	}
          	 	//------------------------------------------------  
          		if(key_ok_bit == KEY_T8)
          	 	{  
          	 	slide_x_deal(3);
          	 	speed_up();
          	 	}
          	 	//------------------------------------------------  
          		if(key_ok_bit == KEY_T9)
          	 	{  
          	 	slide_x_deal(4);
          	 	speed_up();
          	 	}
          	 	//------------------------------------------------  
          		if(key_ok_bit == KEY_T10)
          	 	{  
          	 	slide_x_deal(5);
          	 	speed_up();
          	 	}
          	 	//------------------------------------------------  
          		if(key_ok_bit == KEY_T19)
          	 	{  
          	 	slide_x_deal(6);
          	 	speed_up();
          	 	}
          	 	//------------------------------------------------  
          		if(key_ok_bit == KEY_T20)
          	 	{  
          	 	slide_x_deal(7);
          	 	speed_up();
          	 	}
          	 	//------------------------------------------------  
          		if(key_ok_bit == KEY_T22)
          	 	{  
          	 	slide_x_deal(8);
          	 	speed_up();
          	 	}
          	 	//------------------------------------------------  
          		if((key_ok_bit == KEY_T23)||(key_ok_bit == KEY_T24))
          	 	{  
          	 	slide_x_deal(9);
          	 	speed_up();
          	 	}
          	 	//------------------------------------------------  
          		/*if(key_ok_bit == KEY_yy)
          	 	{  
          	 	Sinc_key_deal();	
          	 	speed_up();
          	 	}*/        	 		           	            	          	         	          	          	        	           	            	          	         	                	          	         		
               	 	...
               	 	if((key_initial_count >99) && (key_initial_count <200)) 
               	 	 {
               	 	 	...
               	 	 	Disp_buf[0]  &=~SEG_P; 				//IT7028S disable []
               	 	 	Disp_buf[1]  &=~SEG_P; 				//IT7028S disable []
               	 	 	Disp_buf[6]  =CHR_BLANK;			//IT7028S disable []
               	 	 	Disp_buf[7]  =CHR_BLANK;			//IT7028S disable []
               	 	 	Disp_buf[12] &=~(SEG_F|SEG_G); 			//IT7028S disable BBQ
               	 	 	Disp_buf[13] =Disp_buf[12];			//IT7028S disable BBQ
               	 	 	Disp_buf[14] =SEG_A; 				//IT7028S disable flex_L flex_R  []
               	 	 	Disp_buf[15] =SEG_A;				//IT7028S disable flex_L flex_R  []
               	 	}
               	 	else      
               	 	 {               	   
               	 			for(i=0;i<16;i++)   Disp_buf[i] =CHR_BLANK;	     	//all blank
               	   			...
               	   			if((idle_count) && (lock_bz == TRUE))  	  //"Lo"			    
               	    			...
               	    			if(Time_dot_bz ==TRUE)
               	    			{
               	    			if(flash_H_1_min) Disp_buf[5] = CHR_H;  //flash "H"  COM5
               	    			if(flash_H_2_min) Disp_buf[4] = CHR_H;  //flash "H"  COM4
               	    			if(flash_H_3_min) Disp_buf[2] = CHR_H;	//flash "H"  COM2
               	    			if(flash_H_4_min) Disp_buf[3] = CHR_H;  //flash "H"  COM3
               	    			}
               	    			Disp_buf[9] =SEG_A|SEG_B;		//IT7028S stable disp onoff	  			           	 		                       	    		    			             	         
               	  	}                                                        
        	}                                                                
            else	//on status                                              
            	{             		
               	   	//if(time_flash_count %2)  {Disp_buf[0] =CHR_BLANK;Disp_buf[1] =CHR_BLANK;}	//timer flash blank
               	   	//else  
               	   	{
               	 	 ...
               	 	 if(temp_8)      		//if timer value is true	 	              	 	
               	 		{
               	 		 	if((lock_bz == TRUE) && (lock_flash_accout <=4)) 		//flash "Lo"and"timer value"
               					{
               			 		...
               			 		{ 
               	 		 		Disp_buf[0] = Display_decode(temp_8 /10);
               	 		 		Disp_buf[1] = Display_decode(temp_8 %10);
               	 		 		}
               	 		}
               	 	 else 				//if timer value is false
               	 	 	{
               	 		 	if(lock_bz == TRUE)  			//display "Lo"
               	    			...
               	    			else if(time_flash_count) {Disp_buf[0] = NUM_0;Disp_buf[1] = NUM_0; }
               	    			else    
               	    				{
               	    				Disp_buf[0] =CHR_BLANK;
               	    				Disp_buf[1] =CHR_BLANK;
               	    				}
               	    		 	/*else 					//display "--"
               	    		 		{
               	    		 		Disp_buf[0] = SEG_G;
               	 		...
               	 		{
               	 			if(timer_list ==1) Disp_buf[0] =CHR_BLANK;
               	 			if(timer_list ==2) Disp_buf[1] =CHR_BLANK;
               	 		}               	 		
               	   	}               	   	
               	  	Disp_buf[5] =ON_disp_zone_x((Warm_status &Select_1_KEY),0,bbq_L_bz,zone_1_level,&change_count,flex_L_bz,Select_1_KEY,flash_H_1_min,zone_1_nopan_time,zone_2_nopan_time,Err_1_data);	 //zone1 normal display
               	        
              	 	Disp_buf[4] =ON_disp_zone_x((Warm_status &Select_2_KEY),1,bbq_L_bz,zone_2_level,&change_count,flex_L_bz,Select_2_KEY,flash_H_2_min,zone_2_nopan_time,zone_1_nopan_time,Err_2_data);	 //zone2 normal display               	
               	  	
               	  	Disp_buf[2] =ON_disp_zone_x((Warm_status &Select_3_KEY),2,bbq_R_bz,zone_3_level,&change_count,flex_R_bz,Select_3_KEY,flash_H_3_min,zone_3_nopan_time,zone_4_nopan_time,Err_3_data);	 //zone3 normal display
               	  	
               	  	Disp_buf[3] =ON_disp_zone_x((Warm_status &Select_4_KEY),3,bbq_R_bz,zone_4_level,&change_count,flex_R_bz,Select_4_KEY,flash_H_4_min,zone_4_nopan_time,zone_3_nopan_time,Err_4_data);	 //zone4 normal display
 
               	        //Disp_buf[0]  <>COM0 
               	        //>>>  
               	        //right up- <-(D131 D10) <-SEG_P               	        
               	        //Disp_buf[0] |=SEG_P;   	//IT7028S disable []
               	        //<<<
               	        //Disp_buf[1]  <>COM1 
               	        //>>>  
               	        //right up| <-(D132 D11) <-SEG_P               	        
               	        //Disp_buf[1] |=SEG_P;        	//IT7028S disable []  
               	        //<<<       	                  	        
               	        //Disp_buf[6]  <>COM6 
               	        //>>> 
               	        //right md- <-(D134 D51) <-SEG_A
               	        //left md-  <-(D133 D52) <-SEG_B  
               	        //left lo-  <-(D138 D53) <-SEG_C
               	        //right md- <-(D135 D54) <-SEG_D
               	        //left md-  <-(D55)      <-SEG_E  
               	        //left lo-  <-(D136 D56) <-SEG_F 
               	        //right md-  <-(D57)     <-SEG_G
               	        //left md-  <-(D137 D58) <-SEG_P                 	                       	         
               	        //Disp_buf[6] = SEG_ALL;  	
               	        //Disp_buf[6] =CHR_BLANK;   	//IT7028S disable []
               	        //<<<
               	        //Disp_buf[7]  <>COM7 
               	        //>>> 
               	        //right md- <-(D59) 	 <-SEG_A
               	        //left md-  <-(D139 D60) <-SEG_B  
               	        //left lo-  <-(D140 D61) <-SEG_C
               	        //right md- <-(D62)      <-SEG_D
               	        //left md-  <-(D142 D63) <-SEG_E  
               	        //left lo-  <-(D64)      <-SEG_F 
               	        //right md-  <-(D141 D65)<-SEG_G
               	        //left md-  <-(D66)      <-SEG_P                 	                       	         
               	        //Disp_buf[7] = SEG_ALL;  
               	        //Disp_buf[7] =CHR_BLANK;   	//IT7028S disable []    
               	        //<<<           	                	                      	        
               	        //Disp_buf[8]  <>COM8 with High bright;Disp_buf[9] COM9 with Low bright
               	        //>>>
               	        //onoff key <-(D67 D68) <-SEG_A ;(D69 D70) <-SEG_B
               	        //lock  key <-(D71 D72) <-SEG_C ;(D73 D74) <-SEG_D
               	        //stop  key <-(D75 D76) <-SEG_E ;(D77 D78) <-SEG_F
               	        //func  key <-(D79 D80) <-SEG_G ;(D81 D82) <-SEG_P
               	        //<<<             	         
               	        Disp_buf[8] =disp08;	
               	        Disp_buf[9] =SEG_ALL &(~disp08);
               	        //
               	        if(time_flash_count ==0)
               	        {
               	        if((key_zone ==Select_1_KEY)&&((Warm_status &Select_1_KEY)==0)) disp_slide_led(zone_1_level);
               	        if((key_zone ==Select_2_KEY)&&((Warm_status &Select_2_KEY)==0)) disp_slide_led(zone_2_level);
               	        if((key_zone ==Select_3_KEY)&&((Warm_status &Select_3_KEY)==0)) disp_slide_led(zone_3_level);
               	        if((key_zone ==Select_4_KEY)&&((Warm_status &Select_4_KEY)==0)) disp_slide_led(zone_4_level);
               	        }
               	        //<<<
               	        //Disp_buf[10] <>COM10 with High brigh;Disp_buf[11]  COM11 with Low bright
               	        //>>>
               	        //slide0  key <-(D83 D84) SEG_A
               	        //slide1  key <-(D85 D86) SEG_B
               	        //slide2  key <-(D87 D88) SEG_C
               	        //slide3  key <-(D89 D90) SEG_D
               	        //slide4  key <-(D91 D92) SEG_E
               	        //slide5  key <-(D93 D94) SEG_F
               	        //slide6  key <-(D95 D96) SEG_G
               	        //slide7  key <-(D97 D98) SEG_H 
               	        Disp_buf[10] =disp10;
               	        Disp_buf[11] =SEG_ALL &(~disp10);	
               	        //<<<
               	        //Disp_buf[12] <>COM12 with High brigh;Disp_buf[13]  COM13 with Low bright
               	        //>>>
               	        //slide8  key <-(D99  D100) SEG_A
               	        //slide9  key <-(D101 D102) SEG_B
               	        //+       key <-(D103 D104) SEG_C
               	        //boost key <-(D105 D106) <-SEG_D ;(D107 D108) <-SEG_E
               	        //bbq   key <-(D109 D110) <-SEG_F ;(D111 D112) <-SEG_G
               	        //timer key <-(D113 D114) <-SEG_P
               	        Disp_buf[12] =disp12;
               	        Disp_buf[13] =SEG_ALL &(~disp12);			
               	        Disp_buf[13] &=~(SEG_F|SEG_G);				//IT7028S disable BBQ
               	        //<<<
               	        //Disp_buf[14] <>COM14 with High brigh;Disp_buf[15]  COM15 with Low bright
               	        //>>>
               	        //timer key <-(D115 D116) <-SEG_A 
               	        //flexr key <-(D117 D118) <-SEG_B ;(D119 D120) <-SEG_C
               	        //flexl key <-(D121 D122) <-SEG_D ;(D123 D124) <-SEG_E  
               	        //left up-  <-(D125 D126) <-SEG_F
               	        //left md-  <-(D127 D128) <-SEG_G  
               	        //left lo-  <-(D129 D130) <-SEG_P
               	        //<<<                 	                     	                      	        
               	        Disp_buf[14] =disp14;  
               	        //Disp_buf[15] =SEG_ALL &(~disp14); 			 
               	        Disp_buf[15] =SEG_A &(~disp14);				//IT7028S disable flex_L flex_R  []
               	           
               	 	if((zone_flash_count ==0) && (timer_status) && (Time_dot_bz ==TRUE))	//display flash timer indicator "."
               	 	{
               	 	  if(Pause_bz)
               	 	  {
               	 	  if(temp_8 == Pause_save_timer_1)  Disp_buf[5] &=~SEG_P;
               	 	  if(temp_8 == Pause_save_timer_2)  Disp_buf[4] &=~SEG_P;
               	 	  if(temp_8 == Pause_save_timer_3)  Disp_buf[2] &=~SEG_P;
               	 	  if(temp_8 == Pause_save_timer_4)  Disp_buf[3] &=~SEG_P;               	 	  	
               	 	  }	
               	 	  else
               	 	  {
               	 	  if(temp_8 == zone_1_timer)  Disp_buf[5] &=~SEG_P;
               	 	  if(temp_8 == zone_2_timer)  Disp_buf[4] &=~SEG_P;
               	 	  if(temp_8 == zone_3_timer)  Disp_buf[2] &=~SEG_P;
               	 	  if(temp_8 == zone_4_timer)  Disp_buf[3] &=~SEG_P;
               	 	  }
               	 	}
                }                   			   
		Disp_buf[3] =CHR_BLANK;  	//IT7028S disable 4#                      
	} 	
//************************************************ 
//Flash_500ms_bz
//************************************************                  	
...
//************************************************  
	 ...
	 //if(zone_5_timer ==0) timer_status &=~Select_5_KEY;
	 }
