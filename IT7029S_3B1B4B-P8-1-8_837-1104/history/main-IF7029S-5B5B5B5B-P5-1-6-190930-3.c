...
void  inc_x_key_level(u8 limit,u8 *count,u8 *zone_flash,u8 *level,u8 *o_level,u16 time1,u16 *time2,u8 select,u16 *tim1,u16 *tim2,u8 *tiao)
{	
  	if(time1 ==0)
  	{
  	  if((*count) ==0)		(*count) =1;  
	      ...
	      {	
        	...
        	{       	  
        	  (*level)++;
        	  if((limit ==0)&&((*level)>7))   (*level) =7;
        	  if((limit ==1)&&((*level)>14))  (*level) =14;
        	}
    		(*count) =2;	
...
void  dec_x_key_level(u8 limit,u8 *count,u8 *zone_flash,u8 *level,u8 *o_level,u16 time1,u16 *time2,u8 select, u16 *tim1,u16 *tim2,u8 *tiao)
{
   	if(time1 ==0)
  	{
  	  if((*count) ==0)		(*count) =1;  
...
void vif_x_set(u8 status,u8 select,u8 *limit,u8 *level1,u16 *time,u16 *o_time,u16 *tim1)
{
    if(status)
    {
    	 (*limit) =1;
...
void doux_x_set(u8 status,u8 select,u8 *limit,u8 *level1,u16 *time,u16 *o_time,u16 *tim1)
{
    if(status)
    {
    	 (*limit) =0;
          	...
          	if((Power_on_bz == TRUE) && (lock_bz == FALSE)&&(key_account == short_press_time) &&(Pause_bz ==0)) 
          	 {
          	 	...
          	 	//------------------------------------------------  
          	 	if((on1_bz)&&(key_ok_bit == KEY_DEC1))  {dec_x_key_level(zone1_step,&count_1_key,&zone_flash_count1,&zone_1_level,&zone_2_level,boost_time_1,&boost_time_2,Select_1_KEY,&zone_1_timer,&zone_2_timer,&tiao_on_1);speed_up();}
          	 	if((on2_bz)&&(key_ok_bit == KEY_DEC2))  {dec_x_key_level(zone2_step,&count_2_key,&zone_flash_count2,&zone_2_level,&zone_1_level,boost_time_2,&boost_time_1,Select_2_KEY,&zone_2_timer,&zone_1_timer,&tiao_on_2);speed_up();}
          	 	if((on3_bz)&&(key_ok_bit == KEY_DEC3))  {dec_x_key_level(zone3_step,&count_3_key,&zone_flash_count3,&zone_3_level,&zone_4_level,boost_time_3,&boost_time_4,Select_3_KEY,&zone_3_timer,&zone_4_timer,&tiao_on_3);speed_up();} 
          		...
          		//------------------------------------------------  
          	 	if((on1_bz)&&(key_ok_bit == KEY_INC1))  {inc_x_key_level(zone1_step,&count_1_key,&zone_flash_count1,&zone_1_level,&zone_2_level,boost_time_1,&boost_time_2,Select_1_KEY,&zone_1_timer,&zone_2_timer,&tiao_on_1);speed_up();} 
          	 	if((on2_bz)&&(key_ok_bit == KEY_INC2))  {inc_x_key_level(zone2_step,&count_2_key,&zone_flash_count2,&zone_2_level,&zone_1_level,boost_time_2,&boost_time_1,Select_2_KEY,&zone_2_timer,&zone_1_timer,&tiao_on_2);speed_up();} 
          	 	if((on3_bz)&&(key_ok_bit == KEY_INC3))  {inc_x_key_level(zone3_step,&count_3_key,&zone_flash_count3,&zone_3_level,&zone_4_level,boost_time_3,&boost_time_4,Select_3_KEY,&zone_3_timer,&zone_4_timer,&tiao_on_3);speed_up();}
          		...
          		//------------------------------------------------ 
          		if(key_ok_bit == KEY_DOUX1)  {doux_x_set(on1_bz,Select_1_KEY,&zone1_step,&zone_1_level,&boost_time_1,&boost_time_2,&zone_1_timer);speed_up();} 
          		if(key_ok_bit == KEY_DOUX2)  {doux_x_set(on2_bz,Select_2_KEY,&zone2_step,&zone_2_level,&boost_time_2,&boost_time_1,&zone_2_timer);speed_up();} 
          		if(key_ok_bit == KEY_DOUX3)  {doux_x_set(on3_bz,Select_3_KEY,&zone3_step,&zone_3_level,&boost_time_3,&boost_time_4,&zone_3_timer);speed_up();} 
          		if(key_ok_bit == KEY_DOUX4)  {doux_x_set(on4_bz,Select_4_KEY,&zone4_step,&zone_4_level,&boost_time_4,&boost_time_3,&zone_4_timer);speed_up();} 
          		//------------------------------------------------ 
          		if(key_ok_bit == KEY_VIF1)   {vif_x_set(on1_bz,Select_1_KEY,&zone1_step,&zone_1_level,&boost_time_1,&boost_time_2,&zone_1_timer);speed_up();} 
          		if(key_ok_bit == KEY_VIF2)   {vif_x_set(on2_bz,Select_2_KEY,&zone2_step,&zone_2_level,&boost_time_2,&boost_time_1,&zone_2_timer);speed_up();} 
          		if(key_ok_bit == KEY_VIF3)   {vif_x_set(on3_bz,Select_3_KEY,&zone3_step,&zone_3_level,&boost_time_3,&boost_time_4,&zone_3_timer);speed_up();} 
          		if(key_ok_bit == KEY_VIF4)   {vif_x_set(on4_bz,Select_4_KEY,&zone4_step,&zone_4_level,&boost_time_4,&boost_time_3,&zone_4_timer);speed_up();} 
          		//------------------------------------------------ 
