	   ...
	   if(Time_dot_bz ==TRUE) Time_dot_bz =FALSE;
            ...
            if(Power_on_bz ==FALSE)  				  //off status
            	...
            	{             		
              	 	...
              	 	Disp_buf[8] =ON_disp_zone_x((Warm_status &Select_2_KEY),1,zone_flash_count2,bbq_L_bz,zone_2_level%10,&change_count,flex_L_bz,Select_2_KEY,flash_H_2_min,zone_2_nopan_time,zone_1_nopan_time,Err_2_data);	 //zone2 normal display               	
               		}
               		if(zone_3_level ==0) {Disp_buf[13] =CHR_BLANK;Disp_buf[10] =CHR_BLANK;} 
               	  	else
               	  	{
               	  	Disp_buf[13] =Display_decode(zone_3_level/10);
               	  	Disp_buf[10] =ON_disp_zone_x((Warm_status &Select_3_KEY),2,zone_flash_count3,bbq_R_bz,zone_3_level%10,&change_count,flex_R_bz,Select_3_KEY,flash_H_3_min,zone_3_nopan_time,zone_4_nopan_time,Err_3_data);	 //zone3 normal display
               		}
               		if(zone_4_level ==0) {Disp_buf[1] =CHR_BLANK;Disp_buf[5] =CHR_BLANK;} 
               	  	else
               	  	{
               	  	Disp_buf[1] =Display_decode(zone_4_level/10);
               	  	Disp_buf[5] =ON_disp_zone_x((Warm_status &Select_4_KEY),3,zone_flash_count4,bbq_R_bz,zone_4_level%10,&change_count,flex_R_bz,Select_4_KEY,flash_H_4_min,zone_4_nopan_time,zone_3_nopan_time,Err_4_data);	 //zone4 normal display
 			}
 			//
