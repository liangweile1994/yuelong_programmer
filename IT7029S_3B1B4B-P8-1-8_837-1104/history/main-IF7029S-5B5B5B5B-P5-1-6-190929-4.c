...
u16 ON_disp_zone_x(u8 warm_disp,u8 index,u8 zone_x_flash,u8 BBQ,u8 level,u8 *change,u8 index_f,u8 select,u8 flash,u8 pan,u8 other_pan,u8 err)
{
        ...
        if(Ms10_bz4 == TRUE)
         {
        	...
        	key_next_deal();
          	...
          	if((Power_on_bz == TRUE) && (lock_bz == FALSE)&&(key_account == short_press_time) &&(Pause_bz ==0)) 
          	 {
          	 	//------------------------------------------------  
          	 	if(key_ok_bit == KEY_ON1) {on_x_set(&on1_bz);speed_up();} 
          	 	if(key_ok_bit == KEY_ON2) {on_x_set(&on2_bz);speed_up();}  
          	 	if(key_ok_bit == KEY_ON3) {on_x_set(&on3_bz);speed_up();} 
          	 	if(key_ok_bit == KEY_ON4) {on_x_set(&on4_bz);speed_up();}           	 	
          	 	//------------------------------------------------  
          	 	if((on1_bz)&&(key_ok_bit == KEY_DEC1))          	
          		{
          		dec_x_key_level(&count_1_key,&zone_flash_count1,&zone_1_level,&zone_2_level,boost_time_1,&boost_time_2,Select_1_KEY,&zone_1_timer,&zone_2_timer,&tiao_on_1);          		
          	       	speed_up();		
          		}
          		//------------------------------------------------  
          	 	if((on2_bz)&&(key_ok_bit == KEY_DEC2))          	
          		{
          		dec_x_key_level(&count_2_key,&zone_flash_count2,&zone_2_level,&zone_1_level,boost_time_2,&boost_time_1,Select_2_KEY,&zone_2_timer,&zone_1_timer,&tiao_on_2);          		
          	       	speed_up();		
          		}
          		//------------------------------------------------  
          	 	if((on3_bz)&&(key_ok_bit == KEY_DEC3))          	
          		{
          		dec_x_key_level(&count_3_key,&zone_flash_count3,&zone_3_level,&zone_4_level,boost_time_3,&boost_time_4,Select_3_KEY,&zone_3_timer,&zone_4_timer,&tiao_on_3);          		
          	       	speed_up();		
          		} 
          		//------------------------------------------------  
          	 	if((on4_bz)&&(key_ok_bit == KEY_DEC4))          	
          		{
          		dec_x_key_level(&count_4_key,&zone_flash_count4,&zone_4_level,&zone_3_level,boost_time_4,&boost_time_3,Select_4_KEY,&zone_4_timer,&zone_3_timer,&tiao_on_4);          		
          	       	speed_up();		
          		}
          		//------------------------------------------------  
          	 	if((on1_bz)&&(key_ok_bit == KEY_INC1))          	
          		{          		
          	       	inc_x_key_level(&count_1_key,&zone_flash_count1,&zone_1_level,&zone_2_level,boost_time_1,&boost_time_2,Select_1_KEY,&zone_1_timer,&zone_2_timer,&tiao_on_1);
          	       	speed_up();		
          		} 
          		//------------------------------------------------  
          	 	if((on2_bz)&&(key_ok_bit == KEY_INC2))          	
          		{          		
          	       	inc_x_key_level(&count_2_key,&zone_flash_count2,&zone_2_level,&zone_1_level,boost_time_2,&boost_time_1,Select_2_KEY,&zone_2_timer,&zone_1_timer,&tiao_on_2);
          	       	speed_up();		
          		}
          		//------------------------------------------------  
          	 	if((on3_bz)&&(key_ok_bit == KEY_INC3))          	
          		{          		
          	       	inc_x_key_level(&count_3_key,&zone_flash_count3,&zone_3_level,&zone_4_level,boost_time_3,&boost_time_4,Select_3_KEY,&zone_3_timer,&zone_4_timer,&tiao_on_3);
          	       	speed_up();		
          		}
          		//------------------------------------------------  
          	 	if((on4_bz)&&(key_ok_bit == KEY_INC4))          	
          		{          		
          	       	inc_x_key_level(&count_4_key,&zone_flash_count4,&zone_4_level,&zone_3_level,boost_time_4,&boost_time_3,Select_4_KEY,&zone_4_timer,&zone_3_timer,&tiao_on_4);
          	       	speed_up();		
          		}
          		//------------------------------------------------ 
          		if(key_ok_bit == KEY_DOUX1)	
          		{         		 
          	        //Warm_func_x_set(key_zone);
          	        speed_up();	
          		} 
          		//------------------------------------------------ 
          		if(key_ok_bit == KEY_VIF1)	
          		{         		 
          	        //BBQ_key_deal();	
          	        speed_up();	
          		} 
          		//------------------------------------------------ 
          		if(key_ok_bit == KEY_BOOST1)	
          		{         		 
          	        //boost_key_deal();	
          	        speed_up();	
          		} 
          	 ...
          	 {      		
          	 	//------------------------------------------------       		 
        		if(key_ok_bit == KEY_TDEC1) {dec_x_timer(Select_1_KEY,&zone_1_timer);speed_up();}
        		if(key_ok_bit == KEY_TDEC2) {dec_x_timer(Select_2_KEY,&zone_2_timer);speed_up();}
        		if(key_ok_bit == KEY_TDEC3) {dec_x_timer(Select_3_KEY,&zone_3_timer);speed_up();}
        		if(key_ok_bit == KEY_TDEC4) {dec_x_timer(Select_4_KEY,&zone_4_timer);speed_up();}
          		//------------------------------------------------       		 
        		if(key_ok_bit == KEY_TINC1) {inc_x_timer(Select_1_KEY,&zone_1_timer);speed_up();}	
        		if(key_ok_bit == KEY_TINC2) {inc_x_timer(Select_2_KEY,&zone_2_timer);speed_up();}
        		if(key_ok_bit == KEY_TINC3) {inc_x_timer(Select_3_KEY,&zone_3_timer);speed_up();}
        		if(key_ok_bit == KEY_TINC4) {inc_x_timer(Select_4_KEY,&zone_4_timer);speed_up();}	          		           		       	 		           	            	          	         	          	          	        	           	            	          	         	                	          	         		
                  }
        	//------------------------------------------------    	  	         	
        }       
//************************************************
            	...
            	{             		
     	 		...
     	 		Disp_buf[0] =temp_16>>8;
               		Disp_buf[4] =temp_16&0x00FF;              	 		               	 		              	    				          	 		
               	  	...
               	  	temp_16=ON_disp_zone_x((Warm_status &Select_1_KEY),0,zone_flash_count1,bbq_L_bz,zone_1_level,&change_count,flex_L_bz,Select_1_KEY,flash_H_1_min,zone_1_nopan_time,zone_2_nopan_time,Err_1_data);	 //zone1 normal display
               		Disp_buf[3] =temp_16>>8;
               		Disp_buf[7] =temp_16&0x00FF;
               		               		
               	        temp_16=ON_disp_zone_x((Warm_status &Select_2_KEY),1,zone_flash_count2,bbq_L_bz,zone_2_level,&change_count,flex_L_bz,Select_2_KEY,flash_H_2_min,zone_2_nopan_time,zone_1_nopan_time,Err_2_data);	 //zone2 normal display               	
               		Disp_buf[15] =temp_16>>8;
              	 	Disp_buf[8] =temp_16&0x00FF;
               		
               		temp_16=ON_disp_zone_x((Warm_status &Select_3_KEY),2,zone_flash_count3,bbq_R_bz,zone_3_level,&change_count,flex_R_bz,Select_3_KEY,flash_H_3_min,zone_3_nopan_time,zone_4_nopan_time,Err_3_data);	 //zone3 normal display               		
               	  	Disp_buf[13] =temp_16>>8;
               	  	Disp_buf[10] =temp_16&0x00FF;
               		
               		temp_16=ON_disp_zone_x((Warm_status &Select_4_KEY),3,zone_flash_count4,bbq_R_bz,zone_4_level,&change_count,flex_R_bz,Select_4_KEY,flash_H_4_min,zone_4_nopan_time,zone_3_nopan_time,Err_4_data);	 //zone4 normal display 			
