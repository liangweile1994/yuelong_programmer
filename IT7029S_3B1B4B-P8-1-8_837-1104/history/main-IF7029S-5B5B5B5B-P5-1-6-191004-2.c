...
const u8 arr_T_Sens_H[16]={ 20,    18,   12,   12, 12,  14,  16,   20,   20,  14,  14, 12,   12,   16,    18, 20};
...
const u8 arr_T_Sens_L[16]={255,255,  6,  8,255,255, 18,    10,20,    16, 255, 255,255,   6,   6, 255};
...
const u8 arr_T_Sens_AH[16]={    22,  22,  20,   18,   22,   15,    17,  19,    20,    18,  16,   22,   24,   20,   18,  16};
...
void on_x_set(u8 *status,u8 *step,u8 *level)
{
	if(*status) {(*status)=FALSE;(*step)=0;}
        else (*status) =TRUE; 
        (*level) =0; 
         buzzer_time = buz_50ms;  
}
...
void vif_x_set(u8 status,u8 select,u8 *limit,u8 *level1,u16 *time,u16 *o_time,u16 *tim1)
{
    if(status)
    {
         ...
         buzzer_time = buz_200ms;
         {
           ...
           else {change_count =select;(*level1) =8;} 
         }
          	...
          	if((Power_on_bz == TRUE) && (lock_bz == FALSE)&&(key_account == short_press_time) &&(Pause_bz ==0)) 
          	 {
          	 	...
          	 	//------------------------------------------------  
          	 	if(key_ok_bit == KEY_ON1) {on_x_set(&on1_bz,&zone1_step,&zone_1_level);speed_up();} 
          	 	if(key_ok_bit == KEY_ON2) {on_x_set(&on2_bz,&zone2_step,&zone_2_level);speed_up();}  
          	 	if(key_ok_bit == KEY_ON3) {on_x_set(&on3_bz,&zone3_step,&zone_3_level);speed_up();} 
          	 	if(key_ok_bit == KEY_ON4) {on_x_set(&on4_bz,&zone4_step,&zone_4_level);speed_up();}           	 	
          	 	//------------------------------------------------  
