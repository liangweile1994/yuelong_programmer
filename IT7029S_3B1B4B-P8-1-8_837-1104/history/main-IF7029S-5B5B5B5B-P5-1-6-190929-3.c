...
void timer_x_set(u8 select,u8 *count,u16 *p,u8 level)
{
         buzzer_time = buz_50ms;  
          	...
          	if((Power_on_bz == TRUE) && (lock_bz == FALSE)&&(key_account == short_press_time) &&(Pause_bz ==0)) 
          	 {
          		...
          		{         		 
          	 	...
          	 	//------------------------------------------------       		 
        		if(key_ok_bit == KEY_TDEC1)
          		{
          	       	//flex_L_key_deal();  
          	       	speed_up();		
          		}
          		//------------------------------------------------       		 
        		if(key_ok_bit == KEY_TINC1)
          		{
          	       	//flex_R_key_deal();  
          	       	speed_up();		
          		}
          		//------------------------------------------------        	 	 
          		if(key_ok_bit == KEY_TIMER1)  {timer_x_set(Select_1_KEY,&time_flash_count1,&zone_1_timer,zone_1_level);speed_up();}   
          		if(key_ok_bit == KEY_TIMER2)  {timer_x_set(Select_2_KEY,&time_flash_count2,&zone_2_timer,zone_2_level);speed_up();}
          		if(key_ok_bit == KEY_TIMER3)  {timer_x_set(Select_3_KEY,&time_flash_count3,&zone_3_timer,zone_3_level);speed_up();}
          		if(key_ok_bit == KEY_TIMER4)  {timer_x_set(Select_4_KEY,&time_flash_count4,&zone_4_timer,zone_4_level);speed_up();}  	 	
          		//------------------------------------------------           		       	 		           	            	          	         	          	          	        	           	            	          	         	                	          	         		
                  }
        	//------------------------------------------------    	  	         	
        }       
