...
void  lock_key_deal(void)
{
     if(lock_bz == FALSE) 
      {
      	if(key_account == mlong_press_time)
				{
      		buzzer_time = buz_200ms;
...
void doux_x_set(u8 status,u8 select,u8 *zone_flash,u8 *limit,u8 *level1,u16 *time,u16 *o_time,u16 *tim1)
{
    if(status)
    {
    	(*limit) =0;
...
//************************************************ 					
        if(Ms10_bz4 == TRUE)
        	...
        	key_next_deal();
          	//------------------------------------------------ 
         	if(key_ok_bit == KEY_ONOFF)  {lock_key_deal();speed_up();} 
          	//------------------------------------------------ 
          ...
          // 	if(key_ok_bit == KEY_F2)  {flex_R_key_deal();speed_up();} 
           ...
           // if((on4_bz)&&(key_ok_bit == KEY_DEC4))  {dec_x_key_level(zone4_step,&count_4_key,&zone_flash_count4,&zone_4_level,&zone_3_level,boost_time_4,&boost_time_3,Select_4_KEY,&zone_4_timer,&zone_3_timer,&tiao_on_4);speed_up();}
          	...
          	//if((on4_bz)&&(key_ok_bit == KEY_INC4))  {inc_x_key_level(zone4_step,&count_4_key,&zone_flash_count4,&zone_4_level,&zone_3_level,boost_time_4,&boost_time_3,Select_4_KEY,&zone_4_timer,&zone_3_timer,&tiao_on_4);speed_up();}
          		//------------------------------------------------ 
          	if(key_ok_bit == KEY_DOUX1)  {doux_x_set(on1_bz,Select_1_KEY,&zone_flash_count1,&zone1_step,&zone_1_level,&boost_time_1,&boost_time_2,&zone_1_timer);speed_up();} 
          	if(key_ok_bit == KEY_DOUX2)  {doux_x_set(on2_bz,Select_2_KEY,&zone_flash_count2,&zone2_step,&zone_2_level,&boost_time_2,&boost_time_1,&zone_2_timer);speed_up();} 
          	if(key_ok_bit == KEY_DOUX3)  {doux_x_set(on3_bz,Select_3_KEY,&zone_flash_count3,&zone3_step,&zone_3_level,&boost_time_3,&boost_time_4,&zone_3_timer);speed_up();} 
          	//if(key_ok_bit == KEY_DOUX4)  {doux_x_set(on4_bz,Select_4_KEY,&zone_flash_count4,&zone4_step,&zone_4_level,&boost_time_4,&boost_time_3,&zone_4_timer);speed_up();} 
          		//------------------------------------------------ 
	   ...
	   if(Time_dot_bz ==TRUE) Time_dot_bz =FALSE;
            ...
            if(Power_on_bz ==FALSE)  				  //off status
               	 	  ...
               	 	  {               	   
               	 			for(i=0;i<16;i++)   Disp_buf[i] =CHR_BLANK;	     	//all blank
               	   			...
               	   			if((idle_count) && (lock_bz == TRUE))  	  //"Lo"			    
               	    			...
               	    			if(Time_dot_bz ==TRUE)
               	    			{
               	    		   	if(flash_H_1_min) Disp_buf[8] = CHR_H;  //flash "H" 
               	    		  	if(flash_H_2_min) Disp_buf[10] = CHR_H;  //flash "H"  
               	    		  	if(flash_H_3_min) Disp_buf[1] =CHR_H;	//flash "H"  
 			...
 			//               	        
									if(Pause_bz) 
									{
											Disp_buf[9] =SEG_B|SEG_C;
											Disp_buf[8] =SEG_E|SEG_F;
											Disp_buf[11] =SEG_B|SEG_C;
											Disp_buf[10] =SEG_E|SEG_F;
											Disp_buf[0] =SEG_B|SEG_C;
											Disp_buf[1] =SEG_E|SEG_F;
